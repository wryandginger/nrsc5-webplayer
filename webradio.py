...
def parse_nrsc5_output(pipe, program):
    ...
    for line in iter(pipe.readline, ""):
        ...
        if tmt_match:
            ...
            # --- REMOVE OLD DUPLICATE COORDINATES ---
            # Extract coordinates (e.g., "_1_2_") from the new filename
            # This looks for a single digit, underscore, single digit between underscores
            coord_match = re.search(r'_\d_\d_', actual_filename)
            if coord_match:
                coord_token = coord_match.group(0)
                # Remove any existing filename in the list that shares these coordinates
                latest_metadata["tmt_files"] = [f for f in latest_metadata["tmt_files"] if coord_token not in f]
            # ----------------------------------------
